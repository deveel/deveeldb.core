...
namespace Deveel.Data.Indexes {
	...
	/// </para>
	/// </remarks>
	public abstract class BlockIndexBase<TKey, TValue> : IIndex<TKey, TValue> where TValue : IComparable<TValue> {
		protected BlockIndexBase() {
			Count = 0;
			IsReadOnly = false;
			Blocks = new List<IIndexBlock<TKey, TValue>>(10);
		}

		protected BlockIndexBase(IEnumerable<IIndexBlock<TKey, TValue>> blocks)
			: this() {
		...
		protected BlockIndexBase(IEnumerable<TValue> values)
			: this() {
		...
		protected BlockIndexBase(IIndex<TKey, TValue> index)
			: this() {
			if (index is BlockIndexBase<TKey, TValue>) {
				// Optimization for when the input list is a BlockIntegerList
				var blockIndex = (BlockIndexBase<TKey, TValue>) index;
				...
				using (var i = index.GetEnumerator()) {
					while (i.MoveNext()) {
						Add(i.Current);
					}
				}
		...
		protected List<IIndexBlock<TKey, TValue>> Blocks { get; private set; }
		...
		/// <summary>
		/// Creates a new <see cref="IIndexBlock{TKey,TValue}"/> for the given implementation.
		/// </summary>
		/// <returns></returns>
		protected abstract IIndexBlock<TKey, TValue> NewBlock();

		/// <summary>
		/// Called when the class decides the given <see cref="IIndexBlock{TKey,TValue}"/> 
		/// is no longer needed.
		/// </summary>
		...
		/// </remarks>
		protected virtual void OnDeleteBlock(IIndexBlock<TKey, TValue> block) {
		}
		...
		/// <param name="block"></param>
		/// <returns></returns>
		private IIndexBlock<TKey, TValue> InsertBlock(int index, IIndexBlock<TKey, TValue> block) {
		...
		private void RemoveBlock(int index) {
			...
			IIndexBlock<TKey, TValue> newPrev = null;
			IIndexBlock<TKey, TValue> newNext = null;
			if (index + 1 < Blocks.Count) {
		...
		private void InsertIntoBlock(TValue value, int blockIndex, IIndexBlock<TKey, TValue> block, long position) {
			...
			if (block.IsFull) {
				...
				IIndexBlock<TKey, TValue> moveTo;
		...
		/// </remarks>
		/// <returns></returns>
		private TValue RemoveFromBlock(int blockIndex, IIndexBlock<TKey, TValue> block, long position) {
		...
		/// <see cref="IIndexBlock{TKey,TValue}"/> within 'blocks' of the block 
		/// that contains the given key value using the <see cref="IIndexComparer{TKey,TValue}"/> 
		/// as a lookup comparator.
		/// </summary>
		/// <param name="key"></param>
		/// <param name="comparer"></param>
		/// <returns></returns>
		private int FindBlockContaining(TKey key, IIndexComparer<TKey,TValue> comparer) {
			if (Count == 0)
				return -1;
		...
		/// the <see cref="IIndexBlock{TKey,TValue}"/> within 'blocks' of the block 
		/// that contains the given key value using the <see cref="IIndexComparer{TKey,TValue}"/> 
		/// as a lookup comparator.
		/// </summary>
		/// <param name="key"></param>
		/// <param name="comparer"></param>
		/// <returns></returns>
		private int FindLastBlock(TKey key, IIndexComparer<TKey,TValue> comparer) {
			if (Count == 0)
				return -1;
		...
		/// <param name="c"></param>
		/// <returns></returns>
		private int FindFirstBlock(TKey key, IIndexComparer<TKey,TValue> c) {
			if (Count == 0) {
				return -1;
			}
		...
		/// <summary>
		/// Uses a binary search algorithm to quickly determine the index of the 
		/// <see cref="IIndexBlock{TKey,TValue}"/> within 'blocks' of the 
		...
		public bool Contains(TKey key, IIndexComparer<TKey,TValue> comparer) {
		...
		public void InsertSort(TKey key, TValue value, IIndexComparer<TKey,TValue> comparer) {
			CheckImmutable();
		...
		public TValue RemoveSort(TKey key, TValue value, IIndexComparer<TKey, TValue> comparer) {
			CheckImmutable();
		...
		public long SearchLast(TKey key, IIndexComparer<TKey, TValue> comparer) {
			int blockIndex = FindLastBlock(key, comparer);
			long sr;
		...
		public long SearchFirst(TKey key, IIndexComparer<TKey, TValue> comparer) {
		...
		public IIndexEnumerator<TValue> GetEnumerator() {
			return GetEnumerator(0, Count - 1);
		}

		public IIndexEnumerator<TValue> GetEnumerator(long startOffset, long endOffset) {
		...
		class Enumerator : IIndexEnumerator<TValue> {
			private readonly BlockIndexBase<TKey, TValue> index;
			private readonly long startOffset;
			private long endOffset;

			private IIndexBlock<TKey, TValue> currentBlock;
			...
			public Enumerator(BlockIndexBase<TKey, TValue> index, long startOffset, long endOffset) {
				this.index = index;
