...
namespace Deveel.Data.Indexes {
	public sealed class IndexSetStore : IDisposable {
		...
		public IIndexSet<SqlObject, long> GetSnapshotIndex() {
			lock (this) {
		...
		public void CommitIndexSet(IIndexSet<SqlObject, long> indexSet) {
		...
		class SnapshotIndexSet : IIndexSet<SqlObject, long> {
			...
			public IEnumerator<IIndex<SqlObject, long>> GetEnumerator() {
				if (indexes == null)
					return EmptyIndex.Cast<IIndex<SqlObject, long>>().GetEnumerator();
			...
			public IIndex<SqlObject, long> GetIndex(int offset) {
				// Create if not exist.
				if (indexes == null) {
		...
		class Index : BlockIndex<SqlObject, long>, IDisposable {
			private List<IMappedBlock> deletedBlocks;

			private bool disposed;

			public Index(IndexSetStore indexSetStore, int indexNumber, int maxBlockSize, IEnumerable<IIndexBlock<SqlObject, long>> blocks)
				: base(blocks) {
			...
			public IEnumerable<IIndexBlock<SqlObject, long>> AllBlocks {
			...
			protected override IIndexBlock<SqlObject, long> NewBlock() {
				AssertNotDisposed();

				return new MappedBlock(this);
			}

			protected override void OnDeleteBlock(IIndexBlock<SqlObject, long> block) {
		...
		public sealed class IndexBlock {
			...
			public IIndex<SqlObject, long> CreateIndex() {
				// Create the MappedListBlock objects for this view
				var blocks = CreateMappedBlocks().Cast<IIndexBlock<SqlObject, long>>();
