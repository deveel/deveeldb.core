...
namespace Deveel.Data.Indexes {
	///<summary>
	/// An implementation of <see cref="BlockIndexBase{TKey,TValue}"/> that stores 
	...
	/// </remarks>
	public class BlockIndex<TKey,TValue> : BlockIndexBase<TKey,TValue> where TValue : IComparable<TValue>, IEquatable<TValue> {
		/// <summary>
		...
		/// <inheritdoc/>
		public BlockIndex(IEnumerable<TValue> values)
			: base(values) {
		}

		/// <inheritdoc/>
		public BlockIndex(IIndex<TKey, TValue> index)
			: base(index) {
		}

		/// <inheritdoc/>
		public BlockIndex(IEnumerable<IIndexBlock<TKey, TValue>> blocks)
			: base(blocks) {
		}

		/// <inheritdoc/>
		protected override IIndexBlock<TKey, TValue> NewBlock() {
			return new Block(512);
		}

		#region Block

		protected class Block : IIndexBlock<TKey, TValue> {
			private long count;
			...
			IIndexBlock<TKey, TValue> IIndexBlock<TKey, TValue>.Next {
				get { return Next; }
			...
			IIndexBlock<TKey, TValue> IIndexBlock<TKey, TValue>.Previous {
				get { return Previous; }
			...
			/// <inheritdoc/>
			public void MoveTo(IIndexBlock<TKey, TValue> destBlock, long destIndex, long length) {
			...
			/// <inheritdoc/>
			public long BinarySearch(TKey key, IIndexComparer<TKey, TValue> comparer) {
				var arr = GetArray(true);
				long low = 0;
				long high = count - 1;
